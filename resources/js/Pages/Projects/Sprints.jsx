...
export default function Sprints ({ show, onClose, children,projectCod }) {
    ...
    return (
        <Portal> 
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-75 transition-opacity duration-300 overflow-y-auto" onClick={onClose}>
                <div className="bg-white rounded-xl shadow-2xl max-w-5xl w-full m-4 transform transition-all duration-500 max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                    <div class="mx-auto px-4 pt-4"> 
                        <h1 class="text-3xl font-extrabold text-gray-800 mb-2 border-b-2 border-indigo-500 pb-2">🚀 Detalhe das Sprints</h1>
                    </div> 
                    <div class="overflow-y-auto flex-grow px-4 pb-4">
                        {isLoading ?(
                            <div class="flex flex-col space-y-6">Carregando... </div>
                        ):
                        (
                            <div class="flex flex-col space-y-6">
                                {sprints.map(sprint =>(
                                    <div key={sprint.COD_VERSAO} className={`w-full bg-white rounded-xl shadow-lg p-6 border-l-8 ${sprint.VER_LIBERA=='S'?'border-green-500':'border-red-500'}`}>
                                        <h2 class="text-2xl font-bold text-gray-900 mb-4 flex items-center">
                                            <span className={`mr-3 ${sprint.VER_LIBERA=='S'?'text-green-500':'text-red-500'} ` }>
                                        ...
                                        <div class="grid grid-cols-2 md:grid-cols-5 gap-4 text-sm text-gray-600 border-t pt-4">
                                            <div>
                                                ...
                                                <span className={`block px-2 py-1 mt-1 ${sprint.VER_LIBERA=='S'?'bg-green-100':'bg-red-100'} text-black-800 font-semibold rounded-lg text-center`} >{sprint.VER_LIBERA=='S'?'Sim':'Não'}</span>
                                            </div>
                                            
                                            <div>
                                                <span class="font-medium text-gray-800">Data de Início </span>
                                                <span class="block mt-1 text-gray-500 italic">{sprint.VER_DATA}</span>
                                            </div>


                                            <div>
                                                <span class="font-medium text-gray-800">Data da Previsão </span>
